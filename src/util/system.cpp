...
const char * const BITCOIN_CONF_FILENAME = "bitranium.conf";
...
#else
    const char* pszModule = "bitranium";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bitranium";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Bitranium";
#else
    // Unix
    return pathRet / ".bitranium";
#endif
#endif
}
